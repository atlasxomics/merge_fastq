...
def test_reads(file_names: List[str]) -> Set[str]:
    ...
    read_ids = {re.search(read_re, name).group().strip("_")
...
def test_extensions(file_names: List[str]) -> Set[str]:
    ...
    if len(extensions) > 1 and extensions not in allowed_extensions:
        log(
            "Multile file extensions detected, please check inputs.",
            "extension warning",
            "warning"
        )
        return ".txt" 
